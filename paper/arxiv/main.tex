...
\item After the rewrite, the construct is out of reach of the linters we tested,
  ...
  ours.
\item The specific refactoring recommended by \texttt{SIM105} preserves runtime
  semantics while placing the rewritten construct outside the rule sets observed
  here; whether a given instance is defective remains a separate question.
\end{itemize}

\paragraph{Research questions.}
\begin{itemize}[nosep,leftmargin=*]
\item \textbf{RQ1} How many except-handler failure-routing candidates occur in
...
\texttt{print()} never does. Thus \texttt{logger.debug()} silences a typed
handler under this syntactic test but not a catch-all handler. The asymmetry is
deliberate: a
...
the failure-routing cases in that set include patterns outside the subfamily this
tool and other except-handler detectors operationalise. We state this as a boundary
...
Section~\ref{sec:rq5} --- were identified during external review. Fourth: callee-local analysis cannot
...
\item \textbf{Construct validity (Section~\ref{sec:rq3}).} The detector
  operationalises the except-handler subfamily only; 64.3\% of fixes in the
  curated reference set lie outside it. The corpus counts and reference-set
  recall are scoped to the pinned records.
...
except-handler candidates in 524{,}229 scanned lines. Four selected syntactic
...
commit \texttt{3b4c567} contains the complete paper artefacts, including the
post-release ledgers and corrected baseline invocation. The v0.8.0 release tag
...
reference set (all rows diff-read).

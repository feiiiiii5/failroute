...
\usepackage{xcolor}
\usepackage{verbatim}
% Table captions sit above their tables; article's default \belowcaptionskip of
% 0pt put each caption's last line directly on the table's top rule.
\setlength{\belowcaptionskip}{6pt}
% In \texttt, write command-line flags as -{}-flag: with T1 Latin Modern the pair
% -- ligates into a single en dash, so --select would print as a one-dash flag.
...
\author{Yufeiyang Chen\thanks{%
  ...
  the drafting and revision of the text. The author takes responsibility for the final work.
  The detector
...
\item After the rewrite, the construct is out of reach of the linters we tested,
  ...
  set, adds no rule that reports it (Section~\ref{sec:rq1}). The project's
  supplementary \texttt{semgrep} rule covers 18 of the 19; citing that figure requires
  stating that the rule is ours.
...
\texttt{pylint}'s W0718 already reaches (W0718 is the id under which
\texttt{pylint} reports the retired W0703 that the baseline enables). \texttt{silent-suppress} stays at 0/19
...
with no signal), \textbf{CONTRACT} (the behaviour appears deliberate and is
...
those are LLM judgements rather than independent defect adjudications. The
...
handlers did. This contrast does not support generalisation.
...
rule can see (the detector's \texttt{exception\_escapes} check looks only for the
exception object in a returned value or a side-channel call; we refer to this as
the \texttt{exception\_escapes} case); \texttt{pydantic/v1/\_hypothesis\_plugin.py:125} is a
...
\paragraph{Another case the handler-local view cannot decide.}
...
consumed, which lies outside the handler. Like the \texttt{exception\_escapes} case above and the three constructed
probes in the Discussion, it is documented
...
\texttt{exception\_escapes} case and state-flag amplification (both in
Section~\ref{sec:control}), they document
...
\item \textbf{Known predicate limits remain in the shipped version.}
  The three constructed probes of the Discussion, and the state-flag
  amplification and \texttt{exception\_escapes} cases of
  Section~\ref{sec:control}, document false-negative and false-positive modes of
...
and 4 a FALSE POSITIVE label from LLM agents. These are judgements from one
labelling process, not confirmed contracts or defects, and the sample does not
...
\url{https://pypi.org/project/failroute/}. RQ1--RQ4 analyse \texttt{failroute} 0.8.0; RQ5 analyses 0.9.1. The pinned

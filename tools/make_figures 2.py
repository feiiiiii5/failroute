...
def figure1(cells):
    ...
    for p, yi, r, h in zip(pts, y, rows, hi):
        ax.annotate('%d/%d' % (r['numerator'], r['denominator']),
                    (h + 1.5, yi), textcoords='data', ha='left', va='center',
                    fontsize=8)
...
def figure3(cells):
    ...
    fig.suptitle('Detection recall on merged fixes that belong to the failure-routing family',
                 fontsize=11, y=0.995)
    ax.text(0.0, -0.02 / (len(body) + 1),
            'Recall %d/%d = %.1f%% (Wilson 95%%: %.1f–%.1f%%) against all %d family fixes; '
            '%d/%d = %.1f%% (Wilson 95%%: %.1f–%.1f%%) against the %d in-scope subset '
            '(out-of-scope rows are marked detected=n/a in paper/merged-pr-recall.csv). '
            'Denominators recounted from the CSV by tools/compute_intervals.py.' % (
                ...
                scope_c['denominator']),
            fontsize=6.8, va='top', wrap=True, transform=ax.transAxes)
    fig.tight_layout()
